...
import dev.wayron.book_tracker_api.book.model.UpdateProductCommand
...
class BookService(private val repository: BookRepository) {
  ...
  fun updateBook(command: UpdateProductCommand): Book {
    logger.info("Updating the book with ID: ${command.id}, with the following information ${command.book}")
    val bookUpdated = command.book
    bookUpdated.updatedAt = Timestamp(System.currentTimeMillis())
    bookUpdated.id = command.id
